...
FROM (
	SELECT 
		*,
		ROW_NUMBER () OVER (PARTITION BY cst_id ORDER BY cst_create_date DESC) ranking_date
...
SELECT 
	...
	CASE
		...
		END sls_due_dt,
	CASE
		WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_quantity * sls_price != sls_sales THEN sls_quantity * ABS(sls_price)
		ELSE sls_sales
	END sls_sales,
	sls_quantity,
	CASE
